# nf-core/taxprofiler and nf-core/createtaxdb paper companion analysis

Supplementary code and notebooks for the nf-core/taxprofiler and nf-core/createtaxdb publication

## Resource usage metrics (benchmarking)

For the resource usage/benchmarking analyses, see the following notebooks:

1. nf-core/createtaxdb
   1. [benchmarking/createtaxdb-data-retrieval-preparation.qmd](benchmarking/createtaxdb-data-retrieval-preparation.qmd)
   2. [benchmarking/createtaxdb-data-auxiliary-files-retrieval.qmd](benchmarking/createtaxdb-data-auxiliary-files-retrieval.qmd)
   3. [benchmarking/createtaxdb-data-benchmarking-execution.qmd](benchmarking/createtaxdb-data-benchmarking-execution.qmd)
   4. [benchmarking/createtaxdb-results-benchmarking-resourceusage.qmd](benchmarking/createtaxdb-results-benchmarking-resourceusage.qmd)
2. nf-core/taxprofiler
   1. [benchmarking/taxprofiler-data-retrieval-preparation.qmd](benchmarking/taxprofiler-data-retrieval-preparation.qmd)
   2. [benchmarking/taxprofiler-data-benchmarking-execution.qmd](benchmarking/taxprofiler-data-benchmarking-execution.qmd)
   3. [benchmarking/taxprofiler-results-benchmarking-resourceusage.qmd](benchmarking/taxprofiler-results-benchmarking-resourceusage.qmd)

## Clinical metagenomics example use casestudy 

For the example clinical metagenomics example use case, see the following notebooks:

1. [clinical_metagenomics/clinical_metagenomics_database_construction.qmd](clinical_metagenomics/clinical_metagenomics_database_construction.qmd)
2. [clinical_metagenomics/clinical_metagenomics_taxprofiler_run.qmd](clinical_metagenomics/clinical_metagenomics_taxprofiler_run.qmd)
3. [clinical_metagenomics/clinical-metagenomics-results-figures.qmd](clinical_metagenomics/clinical-metagenomics-results-figures.qmd)
